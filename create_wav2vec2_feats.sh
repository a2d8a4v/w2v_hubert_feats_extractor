...
ori_data_root="data"
data_root="data/wav2vec_18_lt"
datasets="train_tr_18_lt train_cv_18_lt test_18_lt"

new_feats_dir="wav2vec2_18_lt"
exp_affix="w2v_18_lt"
...
if [ $stage -le 1 ]; then
    ...
    for dataset in $datasets; do
        ...
        python local/wav2vec2_inference.py --data_dir $outdir --new_feats_dir $new_feats_dir --pretrained_model $pretrained_model --exp_affix $exp_affix
        
        dataset=`basename $outdir`
        cat $new_feats_dir/raw_wav2vec2_${exp_affix}_*${dataset}*.scp > $outdir/feats.scp
        cat $outdir/feats.scp | sort > $outdir/feats.scp.tmp; mv $outdir/feats.scp.tmp $outdir/feats.scp
        utils/copy_data_dir.sh $outdir $w2v_outdir
    done
fi
