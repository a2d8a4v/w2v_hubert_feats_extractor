...
parser.add_argument('--new_feats_dir', type=str, default='wav2vec2')
parser.add_argument('--pretrained_model', type=str, default='facebook/wav2vec2-base-960h')
parser.add_argument('--exp_affix', type=str, default='w2v')
...
model = Wav2Vec2Model.from_pretrained(args.pretrained_model).to(device)

for feats_scp in tqdm(feats_scps):
    feat_reader = kaldiio.load_scp(feats_scp)
    split_idx = os.path.basename(feats_scp).split(".")[1]
    new_feats_scp_name = args.new_feats_dir + "/raw_wav2vec2_" + args.exp_affix + "_" + d + "." + str(split_idx)
    new_feats_scp = {}
    ...
    for utt_id, v in feat_reader.items():
        ...
        with torch.no_grad():
            #wav2vec_values_np = model(wav2vec_values_pt).last_hidden_state.detach().numpy()[0]
            wav2vec_values_np = model(wav2vec_values_pt).last_hidden_state.detach().cpu().numpy()[0]
            new_feats_scp[utt_id] = wav2vec_values_np
